...
import { Users, Music, Code, Star, Map, Gamepad2, Film } from 'lucide-react';
...
const MentorCard: React.FC<MentorCardProps> = ({ mentorPair, slots, onClick }) => {
  const { pair, maxSlots } = mentorPair;
          ...
          <article key={idx} className="flex flex-col h-full">
            ...
            <div className="mt-2 space-y-2 text-sm flex-grow">
              <p className="flex items-center">
                <Music className="w-4 h-4 text-blue-800 mr-2" /> {mentor.musicArtist}
              </p>
              <p className="flex items-start">
                <Code className="w-4 h-4 mt-1 text-blue-800 mr-2 flex-shrink-0" /> {mentor.computingArea}
              </p>
              <p className="flex items-center">
                <Star className="w-4 h-4 text-blue-800 mr-2" /> {mentor.celebrity}
              </p>
              <p className="flex items-center">
                <Map className="w-4 h-4 text-blue-800 mr-2" /> {mentor.dreamDestination}
              </p>
              <p className="flex items-center">
                <Gamepad2 className="w-4 h-4 text-blue-800 mr-2" /> {mentor.hobby}
              </p>
              <p className="flex items-center">
                <Film className="w-4 h-4 text-blue-800 mr-2" /> {mentor.favoriteSeries}
              </p>
            </div>
